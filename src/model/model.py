...
class SimulatorGNN(pl.LightningModule):
    ...
    def test_rollout(self, dataset, full_rollout=True, save_path='./', split='valid'):
        ...
        rrmse_error = torch.mean(torch.tensor(rrmse_list))
        rrmse_std = torch.std(torch.tensor(rrmse_list))
        print(f'RRMSE mean: {rrmse_error:.5f}, RRMSE std: {rrmse_std:.5f}')
        ...
        if self.args.plots_flag:
            ...
            boxplot_error(rmse_list, name=f'{split}_rmse_{name_steps}', error='RMSE', save_path=save_path+'/plots/')
            boxplot_error(rrmse_list, name=f'{split}_rrmse_{name_steps}', error='RRMSE', save_path=save_path+'/plots/')
            if self.args.model == 'gnn':
                plot_mse_rollout(mse_list_roll, name=f'{split}_{name_steps}_rollout_mse', with_wandb=False, save_path=save_path)
                plot_mse_rollout_mean_std(mse_list_roll, name=f'{split}_{name_steps}_rollout_mse_mean_std', save_path=save_path, save=True)
                
                # Plot combined and 3D scatter for worst case
                plot_combined(z_net_list[worst_idx], z_gt_list[worst_idx], self.X, self.Y, name=f'{split}_{name_steps}_rollout_combined_worst', with_wandb=False, save_dir=save_path)
                ...
                plot_combined(z_net_list[best_idx], z_gt_list[best_idx], self.X, self.Y, name=f'{split}_{name_steps}_rollout_combined_best', with_wandb=False, save_dir=save_path)
