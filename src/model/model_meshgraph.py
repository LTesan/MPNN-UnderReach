...
class SimulatorMeshGraph(pl.LightningModule):
    def __init__(self, args=None, mp_steps=15, input_size=1, edge_input_size=1, output_size=1, hidden=128, radius=0.05,
                 layers=2, model_dir='checkpoint/simulator.pth', pretrain=None, noise=0.0,
    ...
    def __add_noise(self, graph):
        pos = graph.pos  # shape: [num_nodes, 3] or similar

        # Create noise for the subset of nodes where graph.n == 0
        noise = torch.rand_like(pos) * self.noise  # noise in [0, noise]
        mask = graph.n != 0
        noise[mask.squeeze()] = 0. 
        #  Add noise to positions
        noisy_pos = pos + noise

        # Update graph attributes
        graph.pos = noisy_pos
        graph.x[:, :3] = noisy_pos  # assuming x's first 3 channels correspond to pos
    ...
    def test_rollout(self, dataset, full_rollout=True, save_path='./', split='test'):
        self.eval()
        ...
        # Separate error lists
        rmse_pos_list, rrmse_pos_list = [], []
        rmse_stress_list, rrmse_stress_list = [], []
        ...
        for sim in dataset:
            ...
            if self.mssg_flag:
                ...
                for mssg_snap in out_snaps:
                    mssg_snap_hops_denorm = [
                        self._output_normalizer.inverse(torch.Tensor(mssg_snap_hop))
                        for mssg_snap_hop in mssg_snap
                    ]
            ...
            # Separate position and stress errors
            rmse_pos_list.append(rmse(z_gt[:, :, :3], z_net[:, :, :3]))
            rrmse_pos_list.append(rrmse_inf(z_gt[:, :, :3], z_net[:, :, :3]))

            rmse_stress_list.append(rmse(z_gt[:, :, -1:], z_net[:, :, -1:]))
            rrmse_stress_list.append(rrmse_inf(z_gt[:, :, -1:], z_net[:, :, -1:]))
        ...
        if self.args.plots_flag:
            ...
            rrmse_pos_tensor = torch.tensor(rrmse_pos_list)
            worst_idx = torch.argmax(rrmse_pos_tensor).item()
            best_idx = torch.argmin(rrmse_pos_tensor).item()

            # Print RMSE for best and worst cases
            print(f'Worst RMSE Position: {torch.sqrt(rrmse_pos_tensor[worst_idx]):.4f}')
            print(f'Best RMSE Position: {torch.sqrt(rrmse_pos_tensor[best_idx]):.4f}')

            # Scatter plots 
            plot_3D_scatter_elliptic(z_net_list[worst_idx], z_gt_list[worst_idx], name=f'{split}_worst_scatter', with_wandb=False, save_dir=save_path +'/gifs')
            plot_3D_scatter_elliptic(z_net_list[best_idx], z_gt_list[best_idx], name=f'{split}_best_scatter', with_wandb=False, save_dir=save_path +'/gifs')

            # Make Boxplots  
            boxplot_error(rmse_pos_list, name=f'{split}_rmse_pos_n_steps', error='RMSE POS', save_path=save_path +'/plots/')
            boxplot_error(rrmse_pos_list, name=f'{split}_rrmse_pos_n_steps', error='RRMSE POS', save_path=save_path +'/plots/')
            boxplot_error(rmse_stress_list, name=f'{split}_rmse_stress_n_step', error='RMSE STRESS', save_path=save_path +'/plots/')
            boxplot_error(rrmse_stress_list, name=f'{split}_rrmse_stress_n_step', error='RRMSE STRESS', save_path=save_path +'/plots/')


        # Convert lists to tensors
        rmse_pos_tensor = torch.tensor(rmse_pos_list)
        rrmse_pos_tensor = torch.tensor(rrmse_pos_list)
        rmse_stress_tensor = torch.tensor(rmse_stress_list)
        rrmse_stress_tensor = torch.tensor(rrmse_stress_list)

        # Compute mean errors
        mean_rmse_pos = torch.mean(rmse_pos_tensor)
        mean_rrmse_pos = torch.mean(rrmse_pos_tensor)
        mean_rmse_stress = torch.mean(rmse_stress_tensor)
        mean_rrmse_stress = torch.mean(rrmse_stress_tensor)

        print(f'Mean RMSE Position: {mean_rmse_pos:.6f}')
        print(f'Mean RRMSE Position: {mean_rrmse_pos:.6f}')
        print(f'Mean RMSE Stress: {mean_rmse_stress:.6f}')
        print(f'Mean RRMSE Stress: {mean_rrmse_stress:.6f}')
